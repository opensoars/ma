 ...
 */
module.exports = function processFile(orig_fn, file_data){

  // Is the file already processed?
  // If yes, return the error message so we can log about it
  // and stop executing for this file
  var is_processed = MA_PROCESSED_RE.test(file_data);
  ...
  //
  // This should now return just a single string to log
  //
  if(!table_o || !table_c || !content_o || !content_c){
    var comment_data =
      getCommentData(orig_fn, table_o, table_c, content_o, content_c);

    logErr(comment_data.msg);
    return comment_data.list.forEach(function (comment_data_elem){
      log(comment_data_elem);
    });
  }
  ...
  if(!table_matches)   return logErr(no_table_re   + orig_fn);
  if(!content_matches) return logErr(no_content_re + orig_fn);
