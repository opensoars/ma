...
var TABLE_OPEN_RE = /<!--- TABLE_OF_CONTENTS -->/,
    TABLE_CLOSE_RE = /<!--- \/TABLE_OF_CONTENTS -->/,
    CONTENT_OPEN_RE = /<!--- CONTENT -->/,
    CONTENT_CLOSE_RE = /<!--- \/CONTENT -->/;
 ...
 *
 * @param t_o {bool}  Has table open?
 * @param t_c {bool}  Has table close?
 * @param c_o {bool}  Has table open?
 * @param c_c {bool}  Has table close?
 */
function logCommentData(t_o, t_c, c_o, c_c){
...
function processFile(fn, data){
  ...
  var has_table_open = TABLE_OPEN_RE.test(data),
      has_table_close = TABLE_CLOSE_RE.test(data),
      has_content_open = CONTENT_OPEN_RE.test(data),
      has_content_close = CONTENT_CLOSE_RE.test(data);
  ...
  if(!has_table_open || !has_table_close
      || !has_content_open || !has_content_close){
    logErr(MSGS.bad_tags + fn);
    return log(
      'table open: '
        + cls(has_table_open, has_table_open ? 'green' : 'red'),
      'table close: '
        + cls(has_table_close, has_table_close ? 'green' : 'red'),
      'content open: '
        + cls(has_content_open, has_content_open ? 'green' : 'red'),
      'content close: '
        + cls(has_content_close, has_content_close ? 'green' : 'red')
    );
  }
