...
(function (){
  ...
  if(!MD_FILE){
    ...
    if(md_files === 'could not read dir'){
      logErr(MSGS.could_not_read_md_dir + MD_DIR);
  ...
  md_files.forEach(function (md_file){
    fs.readFile(md_file, 'utf8', function (err, data){
      if(err)
        return logErr('Error while reading file: ' + md_file, err);

      var processed_file = processFile(md_file, data);

      // Is the processed file there?
      if(!processed_file)
        return logErr(MSGS.processed_file_err + 'it is empty');

      if(typeof processed_file !== 'string')
        return logErr(MSGS.processed_file_err + 'it is not a string');

      if(processed_file === 'already processed')
        return logErr(MSGS.processed_file_err + 'it is already processed');

      writeFile(md_file, processed_file);
