...
function processFile(fn, data){
  ...
  sections.forEach(function (section){
    ...
    section.header = '* [**' + section.header + '**](#' + anchor_link + ')';
  ...
  sections.forEach(function (section){
    ...
    table_to_write.push(section.header);

    section.subs.forEach(function (sub){
      table_to_write.push(sub.header);
    });
  ...
  var file_name = 'test_result'
  
  fs.writeFile(file_name, table_to_write, function (err){
    if(err) return logErr('Could not write ' + file_name, err);
    log('Wrote ' + file_name)
  });
